...
def get_masked_lm_output(bert_config, input_tensor, output_weights, positions,
  ...
  """Get loss and log probs for the masked LM (Inpainting)."""
  input_tensor = gather_indexes(input_tensor, positions)

  with tf.variable_scope("pose/inpainting"):
    ## We apply one more non-linear transformation before the output layer.
    ## This matrix is not used after pre-training.
    with tf.variable_scope("transform"):
      input_tensor = tf.layers.dense(
          input_tensor,
          units=int(input_tensor.shape[-1]),
    ...
    output_bias = tf.get_variable(
        "output_bias",
        shape=[FLAGS.length_feature],
        initializer=tf.zeros_initializer())

    ## we don't reuse the embedding table as the ori BERT
    embedding_table = tf.get_variable(
        name='embedding_table_for_output',
        shape=[FLAGS.length_feature, int(input_tensor.shape[-1])],
    ...
    # padding predictions.
    label_ids = tf.reshape(label_ids, [-1, FLAGS.length_feature])
    label_weights = tf.reshape(label_weights, [-1])

    per_example_loss = tf.reduce_mean(tf.square(logits - label_ids), axis=[-1])
    loss = tf.reduce_sum(label_weights * per_example_loss)/(tf.reduce_sum(label_weights)+1e-9)
...
def input_fn_builder(input_files,
  ...
  def input_fn(params):
    ...
    name_to_features = {
        ...
        "masked_lm_ids":
            tf.FixedLenFeature([max_predictions_per_seq, FLAGS.length_feature], tf.float32), ### for inpainting, tf.float32
        "masked_lm_weights": 
